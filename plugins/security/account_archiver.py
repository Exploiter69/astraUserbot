...
async def _cleanup_if_due(now: float) -> None:
    ...
    try:
        ...
        await db.execute(
            ...
            WHERE rowid IN (
                SELECT rowid FROM account_messages
        ...
        await db.execute(
            ...
            WHERE NOT EXISTS (SELECT 1 FROM account_messages m WHERE m.user_id = user_accounts.user_id)
...
async def handle_arch(event):
    ...
    if cmd == "status":
        ...
        rows = await db.fetchall(
            ...
            FROM account_messages m JOIN user_accounts u ON m.user_id = u.user_id
        ...
        for username, first_name, text in rows:
            ...
            content = text[:45] + "..." if len(text) > 45 else text
...
async def handle_track(event):
    ...
    if cmd == "list":
        ...
        display = [f"- {row[0]}" for row in rows] if rows else ["No users are tracked in this group."]
...
async def archive_watcher(event):
    ...
    try:
        ...
        await db.execute(
            ...
            """,
            (event.sender_id, getattr(sender, "username", None), getattr(sender, "first_name", None),
             getattr(sender, "last_name", None), 1 if getattr(sender, "bot", False) else 0,
             1 if getattr(sender, "verified", False) else 0, now, now),
        )
