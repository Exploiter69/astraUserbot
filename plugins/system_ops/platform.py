...
async def handle(event):
    ...
    if cmd == "health":
        ...
        rows = [
            ...
            f"Services: {len(ctx.services)}",
            f"Tasks active: {len(ctx.tasks.active())}",
            f"Database integrity: {'PASS' if integrity else 'FAIL'}",
            f"Commands: {len(COMMANDS)}",
            f"Jobs worker: {'running' if ctx.get('jobs')._started else 'stopped'}",
            f"HTTP: {'ready' if ctx.get('http').session else 'stopped'}",
            f"Search: {'ready' if getattr(ctx.get('search'), '_ready', False) else 'stopped'}",
            f"Isolation: {ctx.get('isolation').assess().backend}",
        ]
        await event.edit(render("HEALTH // PLATFORM", rows, footer="system_ops | health"))
        return
    ...
    if cmd == "plugins":
        ...
        await event.edit(render("PLUGINS // STATE", rows or ["No plugins discovered."], footer="system_ops | plugins"))
        return

    if cmd == "tasks":
        records = ctx.tasks.snapshot()
        rows = [f"{r['state']} · {r['name']} · {r['owner'] or 'unknown'}" for r in records[-20:]]
        await event.edit(render("TASKS // SUPERVISOR", rows or ["No supervised tasks recorded."], footer="system_ops | tasks"))
        return

    if cmd == "jobs":
        jobs = await ctx.get("jobs").list(limit=20)
        rows = [f"{job.state} · {job.type} · {job.id[:12]} · {job.progress:.0%}" for job in jobs]
        await event.edit(render("JOBS // DURABLE", rows or ["No durable jobs recorded."], footer="system_ops | jobs"))
        return

    if cmd == "cache":
        stats = ctx.get("cache").stats()
        rows = [f"{key}: {value}" for key, value in sorted(stats.items())]
        await event.edit(render("CACHE // STATE", rows[:20] or ["No cache statistics available."], footer="system_ops | cache"))
        return
    ...
    if cmd == "diagnostics":
        ...
        metrics = ctx.get("metrics").snapshot()
        report = {
            "timestamp": time.time(),
            "context": ctx.snapshot(),
            "plugins": plugin_manager.snapshot() if plugin_manager else [],
            "commands": len(COMMANDS),
            "tasks": ctx.tasks.snapshot(),
            "jobs": {"count": len(jobs), "states": {state: sum(1 for job in jobs if job.state.value == state) for state in {job.state.value for job in jobs}}},
            "cache": ctx.get("cache").stats(),
            "db_integrity": await ctx.get("storage").integrity_check(),
            "metrics": asdict(metrics),
            "isolation": asdict(ctx.get("isolation").assess()),
    ...
    if cmd == "search":
        if not arg:
            raise CommandError(f"Usage: {config.PREFIX}search <query>")
        results = await ctx.get("search").search(arg, limit=10)
        rows = [f"[{item.source}] {item.title}: {item.snippet}" for item in results]
        await event.edit(render("SEARCH // RESULTS", rows or ["No results."], footer=f"system_ops | search | {len(results)}"))
        return

    if cmd == "reindex":
        counts = await ctx.get("search").rebuild()
        rows = [f"{source}: {count}" for source, count in sorted(counts.items())]
        await event.edit(render("SEARCH // REBUILT", rows, footer="system_ops | reindex"))
        return
    ...
    if cmd == "flags":
        ...
        flags = ctx.get("flags")
        if not parts:
            items = await flags.list()
            rows = [f"{item['name']}: {'ON' if item['enabled'] else 'OFF'}" for item in items]
            await event.edit(render("FLAGS // STATE", rows or ["No feature flags."], footer="system_ops | flags"))
            return
        if len(parts) != 2 or parts[1].lower() not in {"on", "off"}:
            raise CommandError(f"Usage: {config.PREFIX}flags <name> <on|off>")
