...
class CaseService:
    ...
    async def create(self, title: str, summary: str = "") -> str:
        ...
        now = time.time()
        await self.db.execute("INSERT INTO cases(case_id,title,status,summary,created_at,updated_at) VALUES(?,?,?,?,?,?)", (case_id, title, "OPEN", summary, now, now))
        await self.add_timeline(case_id, "CASE_CREATED", title)
        return case_id

    async def get(self, case_id: str) -> dict | None:
        row = await self.db.fetchone("SELECT case_id,title,status,summary,created_at,updated_at,closed_at FROM cases WHERE case_id=?", (case_id.strip(),))
        return dict(row) if row else None

    async def list(self, *, limit: int = 25) -> list[dict]:
        bounded = max(1, min(int(limit), self.MAX_ROWS))
        rows = await self.db.fetchall("SELECT case_id,title,status,summary,created_at,updated_at,closed_at FROM cases ORDER BY updated_at DESC LIMIT ?", (bounded,))
    ...
    async def add_entity(self, case_id: str, entity_id: str, *, role: str = "SUBJECT", note: str = "") -> None:
        ...
        exists = await self.intelgraph.storage.fetchone("SELECT entity_id FROM intel_entities WHERE entity_id=?", (entity_id,))
        if exists is None:
            raise ValueError("IntelGraph entity not found")
        now = time.time()
        await self.db.execute("INSERT OR REPLACE INTO case_entities(case_id,entity_id,role,note,created_at) VALUES(?,?,?,?,?)", (case_id, entity_id, role, note, now))
        await self.db.execute("UPDATE cases SET updated_at=? WHERE case_id=?", (now, case_id))
    ...
    async def add_timeline(self, case_id: str, kind: str, description: str, *, entity_id: str | None = None, observation_id: str | None = None, event_at: float | None = None) -> int:
        if not await self.get(case_id):
            raise ValueError("Case not found")
        now = time.time()
        cursor = await self.db.execute("INSERT INTO case_timeline(case_id,event_at,kind,description,entity_id,observation_id,created_at) VALUES(?,?,?,?,?,?,?)", (case_id, event_at if event_at is not None else now, self._clean(kind.upper(), 64), self._clean(description, self.MAX_TEXT), entity_id, observation_id, now))
        await self.db.execute("UPDATE cases SET updated_at=? WHERE case_id=?", (now, case_id))
        return int(cursor.lastrowid)

    async def timeline(self, case_id: str, *, limit: int = 100) -> list[dict]:
        bounded = max(1, min(int(limit), self.MAX_ROWS))
        rows = await self.db.fetchall("SELECT id,event_at,kind,description,entity_id,observation_id FROM case_timeline WHERE case_id=? ORDER BY event_at,id LIMIT ?", (case_id, bounded))
        return [dict(row) for row in rows]

    async def entities(self, case_id: str, *, limit: int = 100) -> list[dict]:
        bounded = max(1, min(int(limit), self.MAX_ROWS))
        rows = await self.db.fetchall("SELECT ce.entity_id,ce.role,ce.note,e.entity_type,e.canonical_value,e.display_value FROM case_entities ce JOIN intel_entities e ON e.entity_id=ce.entity_id WHERE ce.case_id=? ORDER BY ce.created_at LIMIT ?", (case_id, bounded))
        return [dict(row) for row in rows]
    ...
    async def close_case(self, case_id: str) -> None:
        if not await self.get(case_id):
            raise ValueError("Case not found")
        now = time.time()
        await self.db.execute("UPDATE cases SET status='CLOSED',closed_at=?,updated_at=? WHERE case_id=?", (now, now, case_id))
