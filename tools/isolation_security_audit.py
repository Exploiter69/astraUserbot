...
async def isolated_probe(service: IsolationService) -> dict[str, bool]:
    if not service.assess().enabled:
        return {"actual_isolated_execution": False, "filesystem_boundary": False, "network_isolation": False, "environment_allowlist": False, "subprocess_timeout": False}
    ...
    with tempfile.TemporaryDirectory(prefix="astra-isolation-audit-") as tmp:
        ...
        try:
            code = (
                ...
                "print('NET=' + ('OPEN' if socket.create_connection(('1.1.1.1',80,0.2) if False else ('1.1.1.1',80),0.2) else 'OPEN'))"
            )
            # The network expression above intentionally uses a short socket timeout;
            # connection failure is the expected isolated result.
            result = await service.run(
                ["python3", "-c", code],
                workspace=root,
                timeout=5,
                max_output_bytes=16 * 1024,
                env={"AUDIT_MARKER": "present"},
            )
            stdout = result.stdout
            network_isolated = "NET=" not in stdout
            env_safe = "ENV=MISSING" in stdout and "AUDIT_MARKER" not in stdout
            filesystem_safe = "HOST=HIDDEN" in stdout and "HOME=HIDDEN" in stdout
            ...
            return {
                ...
                "filesystem_boundary": filesystem_safe,
                "network_isolation": network_isolated,
                "environment_allowlist": env_safe,
...
def static_checks() -> dict[str, bool]:
    ...
    return {
        ...
        "resource_limits": "RLIMIT_AS" in isolation and "RLIMIT_CPU" in isolation and "RLIMIT_FSIZE" in isolation and "RLIMIT_NPROC" in isolation,
        ...
        "archive_traversal": ".." in source_text("helpers/archive.py") and "Archive links and special files are not allowed" in source_text("helpers/archive.py"),
