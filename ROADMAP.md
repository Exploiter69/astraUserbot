...
# Phase 14 — Optional Isolation — IMPLEMENTATION COMPLETE

The workload evidence now justifies selective isolation. Same-process plugins remain same-trust; only explicitly classified child workloads cross the reviewed process boundary.

Implemented:

- dedicated Bubblewrap execution boundary;
- restricted subprocess profile with separate namespaces;
- network-disabled isolated children;
...
- security regression suite and live audit.

No fake sandbox claims. Network-requiring operations such as rclone/downloaders remain outside this boundary and retain their own explicit network policy.
...
The live gate requires Bubblewrap and ffprobe on the owner host. An explicitly isolated workload must fail closed when the reviewed backend is unavailable.
